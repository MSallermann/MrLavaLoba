...
def main(input: Input):
    ...
    for flow in range(0, input.n_flows):
        ...
        if (input.a_beta == 0) and (input.b_beta == 0):
            ...
            # MIN AND MAX)
            n_lobes = int(
                np.ceil(np.random.uniform(input.min_n_lobes, input.max_n_lobes, size=1))
            )
    ...
    if input.saveraster_flag == 1:
        ...
        if input.union_diff_file is not None:
            ...
            print(
                "Volume 1 in intersection", Zs1_vol, "Volume 2 in intersection", Zs2_vol
            )
        ...
        for i_thr in range(n_masking):
            if input.masking_threshold[i_thr] < 1:
                ...
                if flag_union_diff:
                    ...
                    print(
                        "Vol 1 in intersect.", Zs1_vol, "Vol 2 in intersect.", Zs2_vol
                    )
        ...
        if len(input.restart_files) > 0:
            ...
            output_thickness_cumulative = input.run_name + "_thickness_cumulative.asc"
