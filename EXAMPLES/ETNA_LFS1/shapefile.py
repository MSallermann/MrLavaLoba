            ...
            if len(self.parts) == 1:
                ...
                for part in self.parts:
                    if ps == None:
                        ...
                        coordinates.append(
                            tuple([tuple(p) for p in self.points[ps:part]])
                        )
                        ps = part
                else:
...
class Reader:
    ...
    def __shape(self):
        ...
        if nPoints:
            record.points = [
                _Array("d", unpack("<2d", f.read(16))) for p in range(nPoints)
            ]
    ...
    def shapeRecords(self):
        ...
        shapeRecords = []
        return [
            _ShapeRecord(shape=rec[0], record=rec[1])
        ...
        for s in self._shapes:
            ...
            if s.shapeType in (1, 11, 21):
                try:
                    ...
                    raise ShapefileException(
                        "Failed to write point for record %s. Expected floats." % recNum
                    )
